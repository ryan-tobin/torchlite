            ...
            for c in range(channels):
                for i in range(out_h):
                    for j in range(out_w):
                        ...
                        pool_region = x_padded[b, c, h_start : h_start + kh, w_start : w_start + kw]
                        output[b, c, i, j] = np.max(pool_region)

        # Create output tensor with backward function
        out = Tensor(output, requires_grad=x.requires_grad, _children=(x,), _op="MaxPool2d")
            ...
            if x.requires_grad:
                    ...
                    for c in range(channels):
                        for i in range(self.out_h):
                            for j in range(self.out_w):
                                ...
                                # Get pool region
                                pool_region = self.x_padded[
                                    b, c, h_start : h_start + self.kh, w_start : w_start + self.kw
                                ]
                                ...
                                grad_padded[b, c, h_start + max_h, w_start + max_w] += out.grad[
                                    b, c, i, j
                                ]
                ...
                if self.ph > 0 or self.pw > 0:
                    if self.ph > 0 and self.pw > 0:
                        grad_input = grad_padded[:, :, self.ph : -self.ph, self.pw : -self.pw]
                    elif self.ph > 0:
