...
class Sigmoid(Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        def _backward():
            if x.requires_grad:
                x.grad = (
                    x.grad + data * (1 - data) * out.grad
                    if x.grad is not None
                    else data * (1 - data) * out.grad
                )
...
class Tanh(Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        def _backward():
            if x.requires_grad:
                x.grad = (
                    x.grad + (1 - data**2) * out.grad
                    if x.grad is not None
                    else (1 - data**2) * out.grad
                )
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        def _backward():
            if x.requires_grad:
                ...
                x.grad = (
                    x.grad + np.dot(jacobian, out.grad.reshape(-1, 1)).reshape(x.shape)
                    if x.grad is not None
                    else np.dot(jacobian, out.grad.reshape(-1, 1)).reshape(x.shape)
                )
