...
class CrossEntropyLoss(Module):
    ...
    def forward(self, logits: Tensor, targets: Tensor) -> Tensor:
        ...
        out = Tensor(loss, requires_grad=logits.requires_grad, _children=(logits,), _op='cross_entropy')

        def _backward():
            if logits.requires_grad:
                grad = (probs - targets_onehot) / logits.shape[0]
                logits.grad = logits.grad + grad * out.grad if logits.grad is not None else grad * out.grad
