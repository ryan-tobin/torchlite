...
def train_gan(generator, discriminator, data, n_epochs, latent_dim, batch_size=64):
    ...
    def bce_loss(pred, target):
        ...
        eps = 1e-8
        loss = -(
            target.data * np.log(pred.data + eps) + (1 - target.data) * np.log(1 - pred.data + eps)
        )
    ...
    for epoch in range(n_epochs):
        for i in range(n_batches):
            ...
            z = tl.Tensor(
                np.random.normal(0, 1, (batch_size_actual, latent_dim)), requires_grad=True
            )
            ...
            if i % 50 == 0:
                print(
                    f"[Epoch {epoch}/{n_epochs}] [Batch {i}/{n_batches}] "
